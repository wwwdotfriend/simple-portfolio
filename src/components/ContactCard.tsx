...
export default function ContactCard({ isOpen, onClose }: ContactCardProps) {
    ...
    return (
        <>
            ...
            >
                <div
                    className={`relative bg-theme-surface border border-theme-hl-high rounded-2xl p-6 md:p-10 mx-4 w-full max-w-md shadow-2xl transition-all duration-300 ${isOpen ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}
                    onClick={(e) => e.stopPropagation()}
                >
                    {/* Close button */}
                    <button
                        onClick={onClose}
                        className='absolute top-4 right-4 text-theme-muted hover:text-theme-text transition-colors duration-200 cursor-pointer'
                    ...
                    <h2 className='text-theme-text text-2xl font-semibold mb-2 text-center'>let's work together!</h2>
                    <p className='text-theme-subtle text-sm text-center mb-8'>feel free to reach out anytime</p>

                    <div className='flex flex-col gap-4'>
                        <a
                            href='mailto:lukasethanparedes@gmail.com'
                            className='flex items-center gap-4 px-5 py-4 rounded-xl bg-theme-hl-low border border-theme-hl-high hover:border-theme-icon-hover transition-all duration-200 group'
                        >
                            <svg className='w-7 h-7 text-theme-icon-hover' fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M22,5V9L12,13,2,9V5A1,1,0,0,1,3,4H21A1,1,0,0,1,22,5ZM2,11.154V19a1,1,0,0,0,1,1H21a1,1,0,0,0,1-1V11.154l-10,4Z"></path></svg>
                            <div>
                                <p className='text-theme-text font-medium group-hover:text-theme-icon-hover transition-colors duration-200'>email me</p>
                                <p className='text-theme-muted text-xs md:text-sm break-all'>lukasethanparedes@gmail.com</p>
                            </div>
                        </a>
